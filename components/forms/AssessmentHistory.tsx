...
const durationTypes = ["days", "weeks", "months", "years"] as [
  string,
  ...string[]
];
...
const AssessmentHistory: React.FC<FormProps> = ({ studentId }) => {
  ...
  const handleFinalSubmit = async () => {
    const values = assessmentForm.getValues();
  ...
  return (
    <Form {...assessmentForm}>
      <form
        onSubmit={assessmentForm.handleSubmit(onSubmit)}
            ...
            <FormItem>
              <div className="mb-4">
                <FormLabel className="text-base">
                  Methods used to diagnose disability and identify functional
                  limitations
                </FormLabel>
                ...
                <FormField
                  ...
                  render={({ field: methodField }) => (
                    ...
                    >
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              ...
                              methodField.onChange(
                                methodField.value?.filter(
                                  (value) => value !== item.id
                                )
                              );
                              assessmentForm.setValue(
                                `methodDates.${item.id}`,
                                ""
                              );
                              if (item.id === "other") {
                      ...
                      {item.id === "diagnostic" &&
                        methodField.value?.includes("diagnostic") && (
                          <div className="ml-4 flex flex-col space-y-2">
                            {["MRI", "CT", "EEG", "X-Ray", "Other"].map(
                              (option) => (
                                <FormField
                                  key={option}
                                  control={assessmentForm.control}
                                  ...
                                  render={({ field: diagnosticField }) => (
                                    <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                      <FormControl>
                                        <Checkbox
                                          checked={diagnosticField.value?.includes(
                                            option
                                          )}
                                          onCheckedChange={(checked) => {
                                            return checked
                      ...
                      {item.id === "other" &&
                        methodField.value?.includes("other") && (
                          <Controller
