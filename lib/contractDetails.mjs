...
async function contractDetails() {
  const file = await fs.readFile(process.cwd() + '/ignition/deployments/chain-31337/deployed_addresses.json', 'utf8');
  const address = JSON.parse(file);

  const abi = await fs.readFile(process.cwd() + '/ignition/deployments/chain-31337/artifacts/EduCare#EduCare.json', 'utf8').then((res) => {
